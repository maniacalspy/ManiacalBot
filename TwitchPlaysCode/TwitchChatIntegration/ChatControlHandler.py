...
class ChatControlHandler(object):
    @classmethod
    async def create(cls, gameTitle):
        self = ChatControlHandler()
        self.IsActive = False
        self.GameInputData = None
        await self.ImportGame(gameTitle)
        return self

    
    async def ImportGame(self, gameTitle):
        GameData = gameTitle + "GameData"
        if GameData not in sys.modules:
            spec = importlib.util.find_spec(GameData)
            if spec is not None:
                self.GameInputData = getattr(importlib.import_module(GameData), GameData)()
                self.IsActive = True
        else:
            self.GameInputData = getattr(sys.modules[GameData], GameData)()
            self.IsActive = True
    
    async def disablehandler(self):
        self.IsActive = False
        if self.GameInputData is not None and hasattr(self.GameInputData, "mysock"):
            await self.GameInputData.TearDown()

    async def GetKeywords(self):
        if self.GameInputData is not None and self.IsActive:
            return await self.GameInputData.GetKeywords()

    async def HandleIntegration(self, ctx):
        if self.GameInputData is not None and self.IsActive:
            await (self.GameInputData).HandleKeyword(ctx)
